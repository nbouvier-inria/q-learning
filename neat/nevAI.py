...
total_reward = 0
try:
    for time_steps in tqdm(range(GENERATIONS - 1)):
        if done:
            i_episode += 1
            try:
                print(f"Reward is {total_reward}, epsilon is {round(EPS_END + (EPS_START - EPS_END) * math.exp(-1. * time_steps / EPS_DECAY), 2)}, gamma is {round((GAMMA_END + (GAMMA_START - GAMMA_END) * math.exp(-1. * time_steps/ GAMMA_DECAY)), 3)} and current best has loss {round(-100*get_loss(best, time_steps), 1)}")
            except NameError:
                pass
            env.reset()
