...
class Genome(object):
    ...
    def save(self, filename:str):
        ...
        with open(filename, 'w') as f:
            dico = self.to_dict()
            print(dico)
            json.dump(dico, f, cls=NpEncoder)
        print(f"Genome saved at {filename}")
    
    def load(self, filename:str):
        """Load the object from filename as a
        JSON file."""
    ...
    def to_dict(self):
        """Return the object as a dictionary. Does
        not include the fitness, nor activation"""
        return {
            'inputs': self._inputs,
